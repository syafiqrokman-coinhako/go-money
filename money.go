...
func defaultMarshalJSON(m Money) ([]byte, error) {
	...
	amount := m.Amount()
	buff := bytes.NewBufferString(fmt.Sprintf(`{"amount": %s, "currency": "%s"}`, amount.String(), m.Currency().Code))
